...
class Member extends CI_Controller {
	...
	{
		$data['tajuk'] = 'Admin - Add Member';
		$id_user = $this->session->userdata('id');
		$data['id_member'] = $this->mod_sb->getId('tb_member', 'id', 'id');
		$data['getProfile'] = $this->mod_sb->mengambil('tb_user', ['id'=>$id_user])->row();
	...
	public function tambah_member()
	{
	    ...
	    if ($this->form_validation->run() == FALSE) {
			...
			if ($q) {
			    ...
			    redirect('Admin/Member/add_outlet','refresh');
			}
	    }
	    // var_dump($data);
	    // die;
	...
	public function ubah_member()
	{
		...
		if ($q) {
		    ...
		    redirect('Admin/Member/edit_member/'.$id_member,'refresh');
		}
	    // var_dump($data);
	    // die;
	}
